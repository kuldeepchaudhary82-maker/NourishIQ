...
import { VictoryChart, VictoryLine, VictoryAxis, VictoryVoronoiContainer, VictoryTheme, VictoryArea } from 'victory-native';
import axios from 'axios';
import { useAuthStore } from '../store/useAuthStore';

const BodyFatChartScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Error fetching bodyfat data');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <View className="px-6 py-4 flex-row items-center border-b border-gray-100">
        ...
        <Text className="text-xl font-bold text-secondary">Body Fat % Trend</Text>
      </View>
          ...
          <View className="items-center">
            ...
            >
              ...
              <VictoryArea
                data={data}
                style={{
                  data: { fill: "#EF9F27", fillOpacity: 0.2, stroke: "#EF9F27", strokeWidth: 3 },
                }}
                animate={{
                  duration: 1000,
                  onLoad: { duration: 500 }
            ...
            <View className="mt-8 bg-gray-50 p-6 rounded-card w-full">
              <View className="flex-row justify-between mb-4">
                <Text className="text-textSecondary font-medium">Initial Body Fat</Text>
                <Text className="text-secondary font-bold">{data[0]?.y}%</Text>
              </View>
              <View className="flex-row justify-between mb-4">
                <Text className="text-textSecondary font-medium">Current Body Fat</Text>
                <Text className="text-secondary font-bold">{data[data.length - 1]?.y}%</Text>
              </View>
              <View className="flex-row justify-between pt-4 border-t border-gray-200">
                <Text className="text-textSecondary font-bold">Total Reduction</Text>
                <Text className="text-accent font-bold">{(data[0]?.y - data[data.length - 1]?.y).toFixed(1)}%</Text>
              </View>
            </View>
          </View>
        ) : (
...
export default BodyFatChartScreen;
